...
def main(fold: int = 0, epochs: int = 3):
    logger = get_logger("fold_train_tcn")
    set_seed(42)
    ...
    for ep in range(epochs):
        ...
        if probs_all:
            ...
            if ap > best:
                best = ap
                torch.save(model.state_dict(), f"artifacts/checkpoints/tcn_fold{fold}_best.pt")
                logger.info(f"saved best TCN fold{fold} checkpoint ap={best:.4f}")
                bad = 0
            else:
                bad += 1
                if bad >= patience:
